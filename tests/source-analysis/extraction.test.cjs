...
async function run({ cached=false, fail=false, empty=false }={}) {
  ...
  const module=loadTs('lib/signals.ts',{
    ...
    './openrouter':{chatJson:async(opts)=>{calls++;prompt=opts;if(fail)throw Error('provider unavailable');return {data:{signals:[]},modelUsed:'mock'};}},
...
test('fingerprints normalize formatting only, preserve distinct substantive claims',()=>{
  const {signalFingerprint}=loadTs('lib/source-analysis.ts',{'./db':{}});
