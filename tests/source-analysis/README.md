...
- `RUN_LOCAL_ANALYSIS_DB=1 node --test tests/source-analysis/postgres.test.cjs`: 10 passed including parent suite. Creates its own temporary **socket-only** PostgreSQL instance using local `initdb`/`pg_ctl`, then removes it; never reads DATABASE_URL or contacts an existing database.
