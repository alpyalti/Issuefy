...
/* "How it works" signal flow — canvas port of design-reference/signal-flow.js.
   A thin glowing light travels monitor → summarize → verify and loops. */

type SrcNode = { a: number; r: number; icon?: number };
type Output = { dx: number; dy: number; icon: number };
...
export default function SignalFlow() {
  ...
  useEffect(() => {
    ...
    if (!root) return;
    const canvas = root.querySelector<HTMLCanvasElement>("#signal-flow");
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    const iconLayer = root.querySelector<HTMLElement>(".flow-icons");
    const iconEls = iconLayer ? Array.from(iconLayer.querySelectorAll<HTMLElement>(".si")) : [];
    ...
    const srcNodes: SrcNode[] = [
      { a: -2.55, r: 0.92, icon: 0 },
      { a: -1.95, r: 0.64 },
      { a: -1.15, r: 0.95, icon: 1 },
      { a: -0.45, r: 0.58 },
      { a: 0.4, r: 0.92, icon: 2 },
      { a: 1.18, r: 0.64 },
      { a: 2.45, r: 0.88, icon: 3 },
    ];
    const feeders = [-0.66, -0.34, 0, 0.34, 0.66];
    const outputs: Output[] = [
      { dx: 0.85, dy: -0.68, icon: 0 },
      { dx: 0.85, dy: 0, icon: 1 },
      { dx: 0.85, dy: 0.68, icon: 2 },
    ];

    let particles: { x: number; y: number; vx: number; vy: number; r: number; a: number; tw: number }[] = [];
    ...
    const KF: [number, number][] = [
      [0.0, -0.06], [0.05, s1], [0.17, s1],
      [0.33, s2], [0.45, s2],
      [0.61, s3], [0.73, s3],
      [0.92, 1.06], [1.0, 1.1],
    ];
    const smooth = (t: number) => t * t * (3 - 2 * t);
    function lightX(phase: number) {
    ...
    function lightPresent(phase: number) {
    ...
    function drawNode(x: number, y: number, r: number, fill: string, glowAmt: number) {
      ctx!.save();
    ...
    function ring(x: number, y: number, r: number, w: number, alpha: number) {
      if (r <= 0 || alpha <= 0) return;
      ctx!.beginPath(); ctx!.arc(x, y, r, 0, Math.PI * 2);
      ctx!.lineWidth = w; ctx!.strokeStyle = `rgba(${COL.accent},${alpha})`; ctx!.stroke();
    }
    function drawShield(cx: number, a: number) {
      const w = 7, h = 9;
      ctx!.save();
      ctx!.beginPath();
      ctx!.moveTo(cx, cy - h);
    ...
    function stageMonitor(x: number, a: number, time: number) {
    ...
    function stageSummarize(x: number, a: number, time: number) {
      const ringR = 14;
      ...
      for (let r = 0; r < 3; r++) {
        ...
        if (typeof rr === "function") rr.call(ctx!, x - w / 2, yy - 1.1, w, 2.2, 1.1);
        else ctx!.rect(x - w / 2, yy - 1.1, w, 2.2);
        ctx!.fillStyle = `rgba(${COL.glow},${(0.55 + 0.45 * cond) * a})`; ctx!.fill();
      }
    }

    function stageVerify(x: number, a: number, time: number) {
    ...
    function render(time: number, lxN: number, present: number) {
    ...
    function frame(ts: number) {
      ...
      const time = (ts - startT) / 1000;
      const phase = (time % CYCLE) / CYCLE;
      const lxN = lightX(phase);
      const present = lightPresent(phase);
      act[0] = act[1] = act[2] = 1;
      stepParticles(1 / 60);
    ...
    render(0, lightX(0.1), lightPresent(0.1));
    let onScreen = false;
    const sync = () => {
      if (onScreen && !document.hidden) { startT = null; startAnim(); }
      else stopAnim();
    };
  ...
  return (
    <div className="flow-row" ref={rowRef}>
      ...
      <div className="flow-icons">
        ...
        <span className="si" data-out="1"><Icon name="Idea01Icon" size={19} stroke={1.6} /></span>
        <span className="si" data-out="2"><Icon name="Alert02Icon" size={19} stroke={1.6} /></span>
      </div>
      ...
      <div className="flow-cell">
        ...
        <div className="cell-text">
          ...
          <div className="t">Read the open web</div>
          <div className="d">Track news, forums, reviews and social across thousands of sources, around the clock.</div>
        </div>
      </div>
      <div className="flow-cell">
        <div className="cell-anim" />
        <div className="cell-text">
          <div className="k">Summarize</div>
          <div className="t">Distill the signal</div>
          <div className="d">AI filters the noise and condenses it into one short, structured brief.</div>
        </div>
      </div>
      <div className="flow-cell">
        <div className="cell-anim" />
        <div className="cell-text">
          ...
          <div className="t">Trace every claim</div>
          <div className="d">Each signal links straight to its original sources, so you can confirm it in one click.</div>
        </div>
      </div>
    </div>
  );
