...
import { ISSUEFY as D } from "@/lib/mock-data";
import { CAT } from "@/lib/types";
import type { SignalItem, SourceItem, WatchItem } from "@/lib/types";
import type { IconName } from "@/components/icons/registry";
import { Icon } from "@/components/icons/Icon";
import { Favicon } from "@/components/signals/Favicon";
import { SignalCard } from "@/components/signals/SignalCard";
import { AddWatchModal } from "@/components/onboarding/AddWatchModal";
import { Onboarding, type OnboardingResult } from "@/components/onboarding/Onboarding";

type MyCo = { name: string; domain: string; color?: string; initials: string };
...
function Sidebar({
  active, setActive, signalCount, savedCount, watchlist, myCompany, onAdd, onDelete,
}: {
  active: string;
        ...
        <div className="watchlist">
          ...
          {competitors.map((w) => (
            <div className="watch-item" key={"c-" + w.label}>
              <span className={"watch-live " + (w.live ? "on" : "")} />
...
/* ---------- Topbar ---------- */
function Topbar({
  title, subtitle, query, setQuery, onCmd, onHelp,
}: {
  title: string;
  subtitle: string;
  query: string;
      ...
      <div className="topbar-right">
        <div className="search">
          ...
          {query ? (
            ...
            <button className="kbd" onClick={onCmd} title="Command menu (⌘K)">⌘K</button>
          )}
        </div>
        <button className="icon-btn lg" title="Setup guide" onClick={onHelp}><Icon name="Idea01Icon" size={18} stroke={1.6} /></button>
        <button className="icon-btn lg" title="Notifications"><Icon name="Notification01Icon" size={19} stroke={1.6} /><span className="bell-dot" /></button>
...
/* ---------- ⌘K Command palette ---------- */
type CmdItem =
  | { type: "nav"; id: string; icon: IconName; title: string; sub: string }
  | { type: "signal"; sig: SignalItem; icon: IconName; title: string; sub: string }
  | { type: "source"; src: SourceItem; title: string; sub: string };

function CommandPalette({ open, onClose, setActive }: { open: boolean; onClose: () => void; setActive: (id: string) => void }) {
  const [q, setQ] = useState("");
  const [sel, setSel] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);

  const ql = q.trim().toLowerCase();
  const navItems: CmdItem[] = NAV.filter((n) => !ql || n.label.toLowerCase().includes(ql)).map((n) => ({
    type: "nav", id: n.id, icon: n.icon, title: n.label, sub: "Jump to " + n.label,
  }));
  const sigItems: CmdItem[] = D.signals
    .filter((s) => ql && (s.title + " " + s.tags.join(" ")).toLowerCase().includes(ql))
    .slice(0, 5)
    .map((s) => ({ type: "signal", sig: s, icon: CAT[s.category].dotIcon, title: s.title, sub: CAT[s.category].label + " · " + s.tags.map((t) => "#" + t).join(" ") }));
  const sourceItems: CmdItem[] = D.recentSources
    .filter((s) => !ql || (s.name + " " + s.headline).toLowerCase().includes(ql))
    .slice(0, 6)
    .map((s) => ({ type: "source", src: s, title: s.headline, sub: s.name + " · " + s.time }));
  ...
  function activate(item: CmdItem | undefined) {
    if (!item) return;
    if (item.type === "nav") { setActive(item.id); onClose(); }
    else if (item.type === "signal") {
      const c = item.sig.category;
      setActive(c === "signal" ? "signals" : c === "competitor" ? "competitors" : c === "opportunity" ? "opportunities" : "risks");
      onClose();
    } else if (item.type === "source") {
      if (item.src.url && item.src.url !== "#") window.open(item.src.url, "_blank");
      onClose();
    }
  }
...
function Tabs({ value, setValue, counts }: { value: string; setValue: (v: string) => void; counts: Record<string, number> }) {
  return (
    <div className="tabs">
      {TABS.map((t) => (
...
/* ---------- Recent sources rail ---------- */
function RecentSources({ onViewAll }: { onViewAll: () => void }) {
  return (
    <section className="rail-card">
      ...
      <div className="rail-sources">
        {D.recentSources.map((s, i) => (
          <a className="rail-source" href={s.url} target="_blank" rel="noopener noreferrer" key={i} onClick={(e) => { if (s.url === "#") e.preventDefault(); }}>
            <Favicon s={s} size={26} />
...
function SourcesPage() {
  ...
  const map = new Map<string, { name: string; kind: string; color: string; initials: string; items: Map<string, SourceItem> }>();
  all.forEach((s) => {
        ...
        {pubs.map((p, i) => (
          <div className="source-pub" key={i}>
            ...
            <div className="source-pub-links">
              {p.items.map((it, j) => (
                <a className="source-link" href={it.url} target="_blank" rel="noopener noreferrer" key={j} onClick={(e) => { if (it.url === "#") e.preventDefault(); }}>
...
type DDOption = { value: string; label: string; dot?: string };
...
function FilterBar({
  dateRange, setDateRange, company, setCompany, topic, setTopic, flagged, setFlagged,
}: {
  dateRange: string;
  ...
  flagged: boolean;
  setFlagged: (fn: (f: boolean) => boolean) => void;
}) {
...
export default function DashboardApp() {
  ...
  const [saved, setSaved] = useState<Set<string>>(() => new Set(D.signals.filter((s) => s.saved).map((s) => s.id)));
  ...
  const [watchlist, setWatchlist] = useState<WatchItem[]>(D.watchlist);
  const [myCompany, setMyCompany] = useState<MyCo | null>(null);
  const [addOpen, setAddOpen] = useState(false);
  const [onboard, setOnboard] = useState(false);

  const companySet = useMemo(() => new Set(watchlist.filter((w) => w.type === "Competitor").map((w) => w.label)), [watchlist]);

  function addWatch(label: string, type: "Competitor" | "Keyword") {
    setWatchlist((prev) => (prev.some((w) => w.label.toLowerCase() === label.toLowerCase()) ? prev : [...prev, { label, type, live: true }]));
  }
  function deleteWatch(label: string) {
    setWatchlist((prev) => prev.filter((w) => w.label !== label));
  }
  function dismissOnboard() {
    try { localStorage.setItem("issuefy_onboarded", "1"); } catch { /* noop */ }
    setOnboard(false);
  }
  function completeOnboard({ me, competitors, keywords }: OnboardingResult) {
    if (me) setMyCompany(me);
    const adds: WatchItem[] = [
      ...competitors.map((c) => ({ label: c.name, type: "Competitor" as const, live: true })),
      ...keywords.map((l) => ({ label: l, type: "Keyword" as const, live: true })),
    ];
    setWatchlist((prev) => {
      const names = new Set(prev.map((w) => w.label.toLowerCase()));
      return [...prev, ...adds.filter((a) => !names.has(a.label.toLowerCase()))];
    });
  }

  // First-run onboarding gate (read localStorage only on the client)
  useEffect(() => {
    try { if (!localStorage.getItem("issuefy_onboarded")) setOnboard(true); } catch { /* noop */ }
  }, []);
  ...
  const counts = useMemo(() => {
    const c: Record<string, number> = { all: base.length, competitor: 0, opportunity: 0, threat: 0 };
  ...
  return (
    <div className="dash">
            ...
            <div className="main-grid">
              <div className="feed">
                ...
                <div className="signal-list">
                  ...
                  {list.map((s) => (
                    <SignalCard key={s.id} sig={s} saved={saved.has(s.id)} leaving={leaving.has(s.id)} onSave={() => toggleSave(s.id)} onDismiss={() => dismiss(s.id)} companySet={companySet} />
                  ))}
                </div>
              </div>
      ...
      {onboard && <Onboarding userName={D.user.name} onClose={dismissOnboard} onComplete={completeOnboard} />}
    </div>
  );
}
