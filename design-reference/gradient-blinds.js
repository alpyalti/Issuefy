/* Issuefy — GradientBlinds (vanilla port of the React Bits component, via ogl).
   Interactive WebGL gradient with vertical "blinds" and a cursor spotlight.
   Auto-inits on #hero-blinds. */
...
export function initGradientBlinds(container, opts = {}) {
  const o = {
    dpr: Math.min(opts.dpr ?? (window.devicePixelRatio || 1), (window.matchMedia && window.matchMedia('(max-width: 768px)').matches) ? 1.5 : 2),
  ...
  // track the spotlight only on devices with a precise pointer (skip touch)
  ...
  const loop = (time) => {
    ...
    if (!o.paused) { try { renderer.render({ scene: mesh }); } catch (e) {} }
  };
  const start = () => { if (!running) { running = true; lastTime = 0; raf = requestAnimationFrame(loop); } };
  const stop = () => { running = false; if (raf) cancelAnimationFrame(raf); raf = 0; };

  // pause the GPU loop whenever the hero is scrolled off-screen or the tab is hidden
...
// ---- Auto-init for the Issuefy hero ----
const el = document.getElementById('hero-blinds');
if (el) {
  initGradientBlinds(el, {
