...
export async function sendBillingNotification(notification: Notification) {
  if (!configuredEnv(process.env.RESEND_API_KEY)) throw new Error("Billing email is not configured");
  const client = new Resend(configuredEnv(process.env.RESEND_API_KEY));
  const messages = {
  ...
  const result = await client.emails.send({
    from: configuredEnv(process.env.RESEND_FROM_EMAIL) || "Issuefy <hello@issuefy.app>",
