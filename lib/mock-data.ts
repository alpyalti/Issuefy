import type { DashboardData, SignalItem, SourceItem } from "./types";

/* Sample dashboard data (B2B SaaS persona), ported from design-reference/data.js.
   Phase 1 only — real data replaces this in later phases. */

const S: Record<string, Omit<SourceItem, "headline" | "time" | "url" | "key">> = {
...
function src(key: keyof typeof S, headline: string, time: string): SourceItem {
  const s = S[key];
  return { ...s, key: key as string, headline, time, url: "#" };
}
...
const signals: SignalItem[] = [
  {
    ...
    context:
      "Announced at 11pm ET with no migration tooling. Reddit sentiment is split: power users like the upside, smaller teams fear bill shock. A credible window to reinforce your flat-rate positioning.",
  ...
  },
  {
    ...
    context:
      "Four fresh G2 reviews and a LinkedIn thread name slow setup and confusing permissions. Your guided onboarding is a direct answer, worth a comparison page and an outbound sequence.",
  ...
  },
  {
    ...
    context:
      "Led by a top-tier fund, with explicit language about 'moving upmarket.' Expect aggressive hiring and a louder marketing presence within two quarters.",
    tags: ["Vega", "Funding"],
  ...
  },
  {
    ...
    context:
      "Mentions of 'auto status updates' and 'AI standups' are up sharply this month. Aligns with a feature you have in beta, so consider accelerating the public launch.",
    tags: ["AI", "Demand"],
  ...
  },
  {
    ...
    context:
      "Their changelog shows approvals now live inside Slack. Your differentiation here narrows; lean on your audit trail and roles depth instead.",
  ...
  },
  {
    ...
    context:
      "Several agency owners are asking for predictable per-seat pricing after surprise usage bills elsewhere. Your model is the punchline, and a targeted landing page could convert.",
    tags: ["Agencies", "ICP"],
  ...
  },
  {
    ...
    context:
      "A handful of Glassdoor posts mention support layoffs. Likely to show up as slower response times, an angle for your 'real human support' messaging if it holds.",
  ...
  },
  {
    ...
    context:
      "Gartner commentary points to teams collapsing overlapping tools. Position your platform breadth as a way to consolidate, not add another seat.",
    tags: ["Market", "Consolidation"],
    sources: [src("gartner", "SaaS consolidation outlook", "4d ago"), src("theinfo", "The great SaaS cull", "3d ago")],
  },
];

const recentSources: DashboardData["recentSources"] = [];
signals.forEach((sig) => sig.sources.forEach((s) => recentSources.push({ ...s, sigCat: sig.category })));

export const ISSUEFY: DashboardData = {
  ...
  summary: {
    ...
    body:
      "Your closest competitor, **Northwind**, quietly shipped usage-based pricing overnight, and early Reddit reaction is split on the migration path. Two enterprise buyers flagged **onboarding friction** with incumbents as a churn reason on G2. And a $40M raise in adjacent infra tooling suggests budget is shifting toward **consolidation** this quarter.",
    moves: [
  ...
  signals,
  recentSources: recentSources.slice(0, 7),
  watchlist: [
